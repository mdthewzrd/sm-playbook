...
- NEW: 9 EMA requirement - fade days must close below daily 9 EMA
- NEW: Range position filter - D0 open can't be above 65% of range (high to low)
"""
...
class BacksidePopScannerV10:
    ...
    def __init__(self, polygon_api_key=None):
        ...
        self.scan_thresholds = {
            ...
            'max_d0_range_position': 65.0,   # D0 open can't be above 65% in range (from highest high to lowest low after high)
    ...
    def calculate_range_position(self, df, setup_idx, euphoric_high):
        ...
        range_high = range_data['high'].max()  # Should be euphoric high
        range_low = range_data['low'].min()    # Lowest low after high
        
        # Calculate D0 open position in this range
        d0_open = df.iloc[setup_idx]['open']
        if range_high <= range_low:  # Avoid division by zero
            return None
    ...
    def analyze_fade_pattern(self, df, setup_idx, euphoric_high):
        ...
        for i, row in fade_data.iterrows():
            if row['is_red_day']:
                ...
                # NEW: Check if this red day closed below 9 EMA
                if not pd.isna(row['ema_9']) and row['close'] < row['ema_9']:
                    has_red_below_ema9 = True
            else:
                consecutive_reds = 0
        
        # NEW: For multiple red days, require at least one closes below daily 9 EMA
        ...
        for i, row in fade_data.iterrows():
            ...
            # NEW: Check if this outlier day closed below 9 EMA
            closed_below_ema9 = not pd.isna(row['ema_9']) and row['close'] < row['ema_9']
            
            # Need at least 2 of 3 outlier conditions AND must close below 9 EMA
    ...
    def scan_single_ticker_v10(self, symbol, start_date, end_date):
        ...
        for idx in range(1, len(df)):
            ...
            scan_criteria = all([
                ...
                # NEW: Range position check - can't open above 65% of range from high to low
            ...
            if scan_criteria:
                setup = {
                    ...
                    'has_red_below_ema9': fade_info.get('has_red_below_ema9', False),
                    'scanner_version': 'V10'
                }
                setups.append(setup)
    ...
    def run_scan_v10(self, start_date=None, end_date=None, tickers=None):
        ...
        print(f"🚀 BACKSIDE POP SCANNER V10 - FADE PATTERN ANALYSIS")
        print(f"📅 Scanning {len(tickers)} tickers from {start_date} to {end_date}")
        print(f"🔧 V10 Enhancements:")
        ...
        if all_setups:
            ...
            print(f"\n🏆 ALL V10 SETUPS:")
            display_cols = ['symbol', 'date', 'trend_atr', 'gap_atr', 'extension_atr', 'fade_type', 'consecutive_reds']
...
if __name__ == "__main__":
    ...
    if not results.empty:
        # Save results to CSV
        results.to_csv('backside_pop_v10_results.csv', index=False)
        print(f"\n💾 Results saved to: backside_pop_v10_results.csv")