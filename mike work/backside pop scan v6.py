"""
Backside Pop Scanner V6 - 9/20 EMA Deviation Bands
Enhanced with proper 9/20 EMA deviation bands from daily chart logic
D0 open/high must be above upper deviation band (EMA9 + 1.0*ATR_9)
"""
...
END_DATE = "2025-09-01"
MAX_WORKERS = 16  # Increased thread count for faster processing
...
class BacksidePopScannerV6:
    """Backside Pop Scanner V6 with 9/20 EMA Deviation Bands"""
    ...
    def __init__(self, polygon_api_key=None):
        ...
        # Enhanced scan thresholds with 9/20 deviation bands
        self.scan_thresholds = {
            'min_trend_atr': 7.0,           
            'min_gap_atr': 0.4,             
            'min_extension_atr': 1.15,       
            'min_range_close_pct': 75.0,    
            'min_volume_multiple': 0.8,     
            'min_change_atr': 0.6,         
            'max_downtrend_slope': 0.0,    
            'min_ema_extension_pct': 0.0,  
            'min_fade_atr': 3,            
            'min_days_since_high': 25.0,       
            'max_days_since_high': 120.0,      
            'min_price': 10.0,              
            'max_price': 1000.0,            
            'min_volume': 7_000_000,
            'require_dev_band_upper': True   # CRITICAL: D0 open/high must be above 9/20 deviation band upper 1
        }
        ...
        self.session.headers.update({'User-Agent': 'backside-scanner-v6'})
    ...
    def calculate_indicators_with_dev_bands(self, df):
        """Calculate indicators with 9/20 EMA deviation bands"""
        ...
        # 9/20 EMA Deviation Bands (matching daily chart logic)
        # ATR calculation for deviation bands (9-period like chart code)
        df['ATR_9'] = df['true_range'].rolling(9, min_periods=3).mean()
        
        # Deviation bands matching your chart code:
        # Upper bands: EMA9 + (multiplier * ATR_9)
        # Lower bands: EMA20 - (multiplier * ATR_9)
        df['dev_band_upper_1'] = df['ema_9'] + 1.0 * df['ATR_9']  # Upper band 1 (like chart du1=1.0)
        df['dev_band_upper_2'] = df['ema_9'] + 0.5 * df['ATR_9']  # Upper band 2 (like chart du2=0.5)
        df['dev_band_lower_1'] = df['ema_20'] - 2.0 * df['ATR_9'] # Lower band 1 (like chart dl1=2.0)
        df['dev_band_lower_2'] = df['ema_20'] - 2.4 * df['ATR_9'] # Lower band 2 (like chart dl2=2.4)
    ...
    def find_trend_and_euphoric_high_fast(self, df, setup_idx):
        ...
        if setup_idx < 20:  # Need minimum data
    ...
    def scan_symbol_optimized(self, symbol, scan_date, lookback_days=400):
        """Optimized single symbol scanning with deviation bands"""
        try:
            ...
            # CRITICAL: D0 open OR high must be above 9/20 deviation band upper 1
            if self.scan_thresholds['require_dev_band_upper']:
                d0_open_above_dev = d_0['open_above_dev_upper_1'] if not pd.isna(d_0['open_above_dev_upper_1']) else False
                d0_high_above_dev = d_0['high_above_dev_upper_1'] if not pd.isna(d_0['high_above_dev_upper_1']) else False
            ...
            setup_data = {
                ...
                # Deviation band metrics
                'd0_open_above_dev_upper_1': d0_open_above_dev,
                'd0_high_above_dev_upper_1': d0_high_above_dev,
    ...
    def run_historical_scan_threaded(self, start_date, end_date):
        """High-performance threaded historical scan with deviation bands"""
        print(f"BACKSIDE POP SCANNER V6 - 9/20 EMA DEVIATION BANDS")
        print(f"Date Range: {start_date} to {end_date}")
        print(f"Tickers: {len(self.ticker_universe)} symbols")
        print(f"Threading: {self.max_workers} workers")
        print(f"Days since high range: {self.scan_thresholds['min_days_since_high']}-{self.scan_thresholds['max_days_since_high']} days")
        print(f"CRITICAL FILTER: D0 open/high must be above 9/20 deviation band upper (EMA9 + 1.0*ATR_9)")
        print("-" * 80)
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"{dev_status} {symbol} {scan_date} - {result['grade']} ({result['total_score']}) | "
                              f"DevBand: ${result['setup_metrics']['d0_dev_band_upper_1']:.2f} | "
                              f"Open: ${result['setup_metrics']['d0_open_price']:.2f} | "
                              f"High: ${result['setup_metrics']['d0_high_price']:.2f}")
        ...
        print(f"\n{'='*80}")
        print(f"SCAN COMPLETE - V6 WITH 9/20 DEVIATION BANDS")
        print(f"{'='*80}")
        ...
        if all_results:
            ...
            # Results table with deviation band info
            print(f"\n{'Date':<12} {'Symbol':<6} {'Score':<5} {'Grade':<5} {'Trend':<6} {'Gap':<5} {'Ext':<5} {'Range':<6} {'Days':<4} {'DevBand':<8} {'Open':<8} {'High':<8} {'Perf':<6}")
            print("-" * 110)
            ...
            for result in all_results:
                ...
                days = result['setup_metrics']['days_since_high']
                dev_band = f"${result['setup_metrics']['d0_dev_band_upper_1']:.2f}"
                d0_open = f"${result['setup_metrics']['d0_open_price']:.2f}"
                d0_high = f"${result['setup_metrics']['d0_high_price']:.2f}"
                ...
                print(f"{date_str:<12} {symbol:<6} {score:<5} {grade:<5} {trend:<6} {gap:<5} {ext:<5} {range_pct:<6} {days:<4} {dev_band:<8} {d0_open:<8} {d0_high:<8} {perf:<6}")
...
def main():
    """Run the V6 scanner with deviation bands"""
    scanner = BacksidePopScannerV6()
    results = scanner.run_historical_scan_threaded(START_DATE, END_DATE)
