"""
Optimized Backside Pop Scanner - High Performance Version
Enhanced with threading, caching, and flexible days-since-high parameters
"""
...
class OptimizedBacksideScanner:
    """High-Performance Backside Pop Scanner"""
    ...
    def __init__(self, polygon_api_key=None):
        ...
        # Enhanced scan thresholds with min/max days since high
        self.scan_thresholds = {
            ...
            'min_fade_atr': 4,            
            'min_days_since_high': 25.0,       # NEW: Minimum days since euphoric high
            'max_days_since_high': 75.0,      # Maximum days since euphoric high
            'min_price': 10.0,              
            'max_price': 1000.0,            
            'min_volume': 1_000_000         
        }
        
        # Grading system (unchanged but optimized)
        self.grading_system = {
            'weights': {
        ...
        self.session.headers.update({'User-Agent': 'backside-scanner-v3'})
    ...
    def calculate_lingua_indicators_vectorized(self, df):
    ...
    def scan_symbol_optimized(self, symbol, scan_date, lookback_days=400):
        """Optimized single symbol scanning"""
        try:
            ...
            df = self.calculate_lingua_indicators_vectorized(df)
            ...
            # Enhanced criteria check with min/max days since high
            passes_criteria = all([
    ...
    def run_historical_scan_threaded(self, start_date, end_date):
        """High-performance threaded historical scan"""
        print(f"OPTIMIZED BACKSIDE POP SCANNER")
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"✓ {symbol} {scan_date} - {result['grade']} ({result['total_score']})")
        ...
        if all_results:
            ...
            # Results table
            print(f"\n{'Date':<12} {'Symbol':<6} {'Score':<5} {'Grade':<5} {'Trend':<6} {'Gap':<5} {'Ext':<5} {'Range':<6} {'Days':<4} {'Perf':<6}")
            print("-" * 80)
            ...
            for result in all_results:
                ...
                print(f"{date_str:<12} {symbol:<6} {score:<5} {grade:<5} {trend:<6} {gap:<5} {ext:<5} {range_pct:<6} {days:<4} {perf:<6}")
...
def main():
    ...
    scanner = OptimizedBacksideScanner()
