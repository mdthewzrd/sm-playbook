...
class BacksidePopScanner:
    ...
    def __init__(self, polygon_api_key=None):
        ...
        self.scan_all_tickers = SCAN_ALL_TICKERS
        self.max_workers = MAX_WORKERS
        
        # Scan thresholds - LOWER VALUES FOR BETTER DETECTION
        self.scan_thresholds = {
            'min_trend_atr': 3.0,           # Reduced from 5.0
            'min_gap_atr': 0.2,             # Reduced from 0.3
            'min_extension_atr': 0.3,       # Reduced from 0.5
            'min_range_close_pct': 30.0,    # Reduced from 40.0
            'min_volume_multiple': 0.6,     # Reduced from 0.8
            'min_change_atr': 0.2,          # Reduced from 0.3
            'max_downtrend_slope': -0.3,    # Less strict (closer to 0)
            'min_ema_extension_pct': 3.0,   # Reduced from 5.0
            'min_fade_atr': 1.5,            # Reduced from 2.0
            'min_days_since_high': 1,       
            'max_days_since_high': 90,      # Increased from 60
            'min_price': 5.0,               # Reduced from 10.0
    ...
    def run_historical_scan(self, start_date, end_date):
        """Run historical scan with threading and display results"""
        print(f"BACKSIDE POP HISTORICAL SCAN")
        print(f"Date Range: {start_date} to {end_date}")
        
        if self.scan_all_tickers:
            print(f"Scanning ALL market tickers (threaded with {self.max_workers} workers)")
        else:
        ...
        for date_idx, date in enumerate(trading_dates):
            print(f"\nProcessing date {date_idx + 1}/{len(trading_dates)}: {date}")
            
            # Get ticker list for this date
            if self.scan_all_tickers:
                current_tickers = self.fetch_all_market_tickers(date)
                print(f"Got {len(current_tickers)} tickers for {date}")
            else:
                current_tickers = self.ticker_universe
            
            # Create tasks for this date
            date_tasks = [(ticker, date) for ticker in current_tickers]
            date_results = []
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all tasks
                future_to_task = {
                    executor.submit(self.scan_symbol, ticker, date): (ticker, date)
                    for ticker, date in date_tasks
                }
                
                # Process completed tasks
                completed = 0
                for future in as_completed(future_to_task):
                    ticker, scan_date = future_to_task[future]
                    completed += 1
                    ...
                    try:
                        ...
                        if result:
                            ...
                            print(f"Found: {ticker} {scan_date} - Grade: {result['grade']} (Score: {result['total_score']})")
                        
                        # Progress update every 100 completions or at end
                        if completed % 100 == 0 or completed == len(date_tasks):
                            progress = (completed / len(date_tasks)) * 100
                            print(f"  Date progress: {completed}/{len(date_tasks)} ({progress:.1f}%) - {len(date_results)} setups found")
                            
                    except Exception as e:
                        # Skip individual ticker errors
                        pass
            
            # Add date results to all results
            all_results.extend(date_results)
            print(f"Date {date} complete: {len(date_results)} setups found")
        ...
        if all_results:
            ...
            # Performance summary
            if any(r['performance'] and r['performance'].get('swing_fade_5d_pct') for r in all_results):
                ...
                success_rate = len([r for r in perf_results if r['performance']['swing_fade_5d_pct'] < -5]) / len(perf_results) * 100
                print(f"Average 5-day performance: {avg_perf:.1f}%")
